...
func (p *processorImp) GetCapabilities() component.ProcessorCapabilities {
	p.logger.Info("GetCapabilities for signozspanmetricsprocessor")
...
// into the given instrumentation library metrics.
func (p *processorImp) collectDBCallMetrics(ilm pdata.InstrumentationLibraryMetrics) {
	for key := range p.dbLatencyCount {
		...
		dpLatency.SetSum(int64(p.dbLatencySum[key]))

		dpLatency.LabelsMap().InitFromMap(p.dbMetricKeyToDimensions[key])
	}
}

// collectExternalCallMetrics collects the raw latency metrics, writing the data
// into the given instrumentation library metrics.
func (p *processorImp) collectExternalCallMetrics(ilm pdata.InstrumentationLibraryMetrics) {
	for key := range p.externalCallLatencyCount {
		...
		dpLatency.SetSum(int64(p.externalCallLatencySum[key]))

		dpLatency.LabelsMap().InitFromMap(p.externalCallMetricKeyToDimensions[key])
	}
}
...
func buildCustomDimensionKVs(serviceName string, span pdata.Span, optionalDims []Dimension) dimKV {
	dims := make(dimKV)
...
func buildCustomKey(serviceName string, span pdata.Span, optionalDims []Dimension) metricKey {
...
// dbCache the dimension key-value map for the metricKey if there is a cache miss.
...
func (p *processorImp) externalCallCache(serviceName string, span pdata.Span, k metricKey) {
	if _, ok := p.externalCallMetricKeyToDimensions[k]; !ok {
		p.externalCallMetricKeyToDimensions[k] = buildCustomDimensionKVs(serviceName, span, p.externalCallDimensions)
	}
}
